...
function jaca_wars(v, theta) {
	let g = 9.8;
	let dist = (Math.pow(v, 2) * Math.sin(2 * theta)) / g;
	if (dist < 100 && dist + 2 < 100) {
		return -1;
	}
	if (dist > 100 && dist - 2 > 100) {
		return 1;
...
const vol_pizza = (raio_z, altura) =>
	Math.round(Math.PI * Math.pow(raio_z, 2) * altura);
...
// EXERCÍCIO soma-segundo-maior-e-menor-numeros
// Maior e menor
function soma_maior_e_menor(numbers) {
	let num_anterior = 0;
	let num;
	let max = numbers[i];
	let min = numbers[i];
	...
	for (let j = 0; j < numbers.length; j++) {
		num2 = numbers[j];
		if (num2 < max && num2 > num_anterior2) {
			max2 = num2;
		}
		if (num2 > min && num2 < num_anterior2) {
			min2 = num2;
		}
		num_anterior2 = num2;
	}
	...
	.then((res) => {
		...
		axios
			...
			.then((res) => {
				...
				// EXERCÍCIO TAM STRING
				let entrada1_tamstring =
					exercicios["tamanho-string"]["entrada"]["string"];
				...
				let entrada1_nomeuser =
					exercicios["nome-do-usuario"]["entrada"]["email"];
				...
				// EXERCÍCIO jaca-wars
				let entrada1_jacawars = exercicios["jaca-wars"]["entrada"]["v"];
				let entrada2_jacawars = exercicios["jaca-wars"]["entrada"]["theta"];
				let resultado_jacawars = jaca_wars(
					entrada1_jacawars,
					entrada2_jacawars
				);
				console.log(resultado_jacawars);
				axios.post(
					"https://tecweb-js.insper-comp.com.br/exercicio/jaca-wars",
					{ resposta: resultado_jacawars },
					options2
				);
				...
				let resultado_volpizza = vol_pizza(
					entrada1_volpizza,
					entrada2_volpizza
				);
				...
				// EXERCÍCIO inverte-string
				let entrada1_invertestr =
					exercicios["inverte-string"]["entrada"]["string"];
				...
				// EXERCÍCIO soma-segundo-maior-e-menor-numeros
				// let entrada1_soma2maioremenor = exercicios["soma-segundo-maior-e-menor-numeros"]["entrada"]["numeros"];
				// let resultado_soma2maioremenor = soma_maior_e_menor(entrada1_soma2maioremenor);
				// console.log(resultado_soma2maioremenor);
				// axios
				//     .post(
				//         "https://tecweb-js.insper-comp.com.br/exercicio/soma-segundo-maior-e-menor-numeros",
				//         {"resposta": resultado_soma2maioremenor },
				//         options2
				//     );

				// EXERCÍCIO caca-ao-tesouro
				// let entrada1_tesouro = exercicios["caca-ao-tesouro"]["entrada"]["inicio"];
				// axios.get(
				//     `${entrada1_tesouro}`,
				//     options2
				// ).then((tesouro) => {
				//     if (tesouro)
				// })
				// let resultado_tesouro = soma_obj(entrada1_tesouro);
				// console.log(resultado_tesouro);
				// axios
				//     .post(
				//         "https://tecweb-js.insper-comp.com.br/exercicio/caca-ao-tesouro",
				//         {"resposta": resultado_tesouro },
				//         options2
				//     );
			});
	});
